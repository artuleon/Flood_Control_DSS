...
function f = Fitness_vectorized_Pareto(x)
...
global EnterDateofsimulationHECHMS
...
f = zeros(number_rows_objective,Number_of_objectives);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Writing HEC-DSS files using HEC-DSSVue
...
%for RAS_simul_ID = 1:number_rows_objective
        ...
        while kk <= NumberWetlands_managed
                ...
                if status ~= 0
                    disp('Error HEC-DSSVue.exe. Fitness vectorized_Pareto')
...
for RAS_simul_ID = 1:number_rows_objective        
        StrID = num2str(RAS_simul_ID);
        InputDSS_HMS = [path_general '\RAS_HMS_folders\' General_Name_of_Project StrID '\' string_Home_folder_HMS];
        temp_file = [InputDSS_HMS '\Run_1.dss'];
        ...
        if  status == 0; %Status = 0 (successful run), status = 1 (aborted run). If aborted, don't run HEC-RAS
                ...
                !taskkill /im hec-hms.cmd  
                folderName = strcat(General_Name_of_Project,StrID); 
                disp('Unsuccesful HEC-HMS simulation. Fitness_vectorized_Pareto')            
...
if number_rows_objective_modif >= 1
    ...
    Objective_water_constraint = zeros(number_rows_objective_modif, 1); 
    parfor kk = 1:number_rows_objective_modif        
    %for  kk = 1:number_rows_objective_modif
                Penal = zeros(1,number_of_columns);
                Flow_constraint = zeros(1,number_of_columns);
                ...
                if wascomputationcompleted(kk) == 1; %Simulation of entire population was completed                      
                        ...
                        for j = 1:number_of_cross_sections_constraints
                                ...
                                FindOnesLocation= find(z7_high_logic==1);
                                CountOnes = length(FindOnesLocation);
                                                                
                                water_level_exceeded_objective(kk) = sum((z7_high-Max_water_level_check_constraint(j)).^2.0);
                                ii = 1
                                while ii <= NumberWetlands_managed 
                                    ...
                                    hours_hydro = 8; %Define this for every wetland
                                    for jj = 1:CountOnes;
                                        MinLocat = max(1,FindOnesLocation(jj)-hours_hydro);
                                        MaxLocat = FindOnesLocation(jj);
                                        z7_high_logic(MinLocat:MaxLocat) = 1;
                                    end  
                                    Flow_constraint(1,col1+1:col2) = z7_high_logic(2:N+1);
                                    ii = ii + 1;
                                end 
                ...
                %Objective_Results(kk) = -dot(x(RAS_simul_ID,:),Penal(1,:));  This is negative               
                Objective_Results(kk) = dot(x(RAS_simul_ID,:),Flow_constraint(1,:)); %This is positive
                %Objective_water_constraint(kk) = water_level_exceeded_objective
                %The sign is negative because we are minimizing
                wasRASCOMPLETED_O_MEANS_NO = wascomputationcompleted(kk)
    end
    
    for kk = 1:number_rows_objective_modif 
        RAS_simul_ID = ID_RAS_simulations(kk);        
        if Number_of_objectives == 1
            f(RAS_simul_ID,1) = water_level_exceeded_objective(kk);
        elseif Number_of_objectives == 2
            f(RAS_simul_ID,1) = water_level_exceeded_objective(kk);            
            f(RAS_simul_ID,2) = Objective_Results(kk);
        else
           msg = 'Number of objectives larger than 2'; 
            error(msg) 
        end
    end   
